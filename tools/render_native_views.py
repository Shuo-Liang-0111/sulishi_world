...
if version in ['G1_027r8','G1_027r9','G1_027r10','G1_027r11']:
...
try:
    ...
    if version in ['G1_027r9','G1_027r10','G1_027r11']:
    ...
    if version in ['G1_027r10','G1_027r11']:
    ...
    for index,camera in enumerate(args):
        if index:
            ...
            sf1_view=camera.startswith('SF1_QA_')
            runpy.run_path(str(root/'tools/blender_render_bellevue.py'),init_globals={'REVIEW_CAMERA':camera,'REVIEW_DEVICE':'CPU','REVIEW_SAMPLES':32 if sf1_view else 24,'REVIEW_RESOLUTION':(1400,960) if sf1_view else (1280,840)})
        ...
        completed.append({'camera':camera,'file':str(path.relative_to(root)),**verified,
                          'stadelhofen_door_open_fraction':float(bpy.data.objects['SF1_DOOR_CENTRE_CONTROL']['open_fraction']) if version=='G1_027r11' else None,
                          'sternen_door_open_fraction':float(bpy.data.objects['SG_R8_RESTAURANT_DOOR_CONTROL']['open_fraction']) if version in ['G1_027r8','G1_027r9','G1_027r10','G1_027r11'] else None})
