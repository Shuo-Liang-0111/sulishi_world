"""Save027r2 separately after actual bridge and neighbouring-place checks."""
from pathlib import Path
import contextlib,hashlib,io,json,runpy,shutil
import bpy
R=Path('F:/MyWorld/ZurichWorld');s=bpy.context.scene;E=R/'evidence/G1_027r2'
assert s['version']=='G1_027r2' and Path(bpy.data.filepath).name=='G1_027r1_bridgehead_grade_working.blend'
target=R/'native/G1_027r2_bridge_surface_refinement_working.blend';assert not target.exists()
assert shutil.disk_usage(R).free>350_000_000
checks=['blender_check_bridge_grade_refinement.py','blender_check_bridge_deck.py','blender_check_quaibruecke.py',
...
record=json.loads((R/'evidence/G1_027r1/checkpoint.json').read_text())
...
record.update(version=s['version'],native=target.relative_to(R).as_posix(),native_bytes=target.stat().st_size,native_sha256=digest,
    ...
    grade_refinement_file=s['grade_refinement_file'],grade_refinement_sha256=hashlib.sha256((R/s['grade_refinement_file']).read_bytes()).hexdigest(),
    pre_save_checks=checks)
(E/'checkpoint.json').write_text(json.dumps(record,indent=2),encoding='utf-8')
print('BRIDGE_REFINEMENT_DRAFT_SAVED',target.stat().st_size,digest,flush=True)
