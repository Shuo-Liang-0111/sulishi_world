...
pavements=[o for o in s.objects if o.type=='MESH' and not o.name.startswith(('CTX_','I3S_'))
           and any(t in o.name for t in ['PAVING','SURVEY_TERRAIN','STREET_APPROACH']) and intersects(o)]
assert context
rows=[];trees=[]
...
p=write_path(f'derived/bellevue/bank_tram_shelter/native_probe_{s["version"]}.json')
