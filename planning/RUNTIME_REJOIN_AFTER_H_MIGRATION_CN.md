...
当前原生为`native/G1_027r15_bank_eave_scan_repair.blend`，SHA256 `496397518d867f64a0ea2f8052b0247881ef0bbd9c5aa570a9857e71e6cf2b9d`。组件PID31924于18:30:34 UTC实际退出0、stderr为空，原生没有重存。新增SF1的相机不在旧相机集合；清单初次93台的结果保留，枚举所有场景相机后PID16688独立运行成功，正确清点99台。239个材质、13,510可见作者对象、39,057,216多边形，不能直接当成已完成实时导出的清单。
...
精确方案该树属性存储由23,385,600降至12,115,590字节，减少约48.19%。这只是属性字节计数，不是GPU计时、整场景内存或帧率结果。两边试验同用MeshStandardMaterial，尚未实现原生0.045 SSS与精确位置对应的阴影/depth材质，不能直接发布为完整树木运行时。其他树种和全体树木也未验证。证据与实际画面见`evidence/G1_027r15/leaf_exact_numeric_roundtrip.json`、`leaf_exact_browser_report.json`、`leaf_exact_visual_review.json`、`leaf_exact_browser_near.png`。
...
4. 相机不能只导出BE/HB前缀或只检查`90_REVIEW_CAMERAS`；新增SF1集合也拥有正式机位。枚举全场景相机并记录所属集合，保留原机位、实际眼高、色彩设置及原生文件身份，避免默认首页指向旧版。
