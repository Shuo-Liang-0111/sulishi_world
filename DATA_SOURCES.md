...
补读缓存AV456/457，确认它们是与AV355相接的两个交通岛端部，合成285.910087m²连续平面；没有改动真实XY。初次纵断面未包含端岛支持，导致不受约束的下凹，已保留失败候选；随后读取12块原摄影、376个地面候选面，完整1142点中928处有支持。第二候选最差负路缘高差从约-167mm降到-62.6mm，此版仍不能施工，也不能据此直接认定旧道路错误。tbl_routennetz.3806的沿站台折线交点没有被误标为新增过街坡道。

进一步从r10原生实际读出46个相关对象与949条射线；其中6个道路对象的3251个邻近三角面与历史道路配方一致到0.000665mm，源指纹保留于current_road_support.json。以真实摄影和当前道路共同约束后，最新准备地坪没有负高差，拟合RMS32.9mm，端口误差最大0.289mm。7mm边缘最低余量、4mm端口余量、局部平滑过渡及制造参数明确为推断，不能当作市政实测值。尚未建造新站台/站棚或修改对应摄影。详见planning/BELLEVUE_BANK_TRAM_SHELTER_CN.md与版本化原生探针。

## Bellevue候车设施与摄影残片027r12/13

2026-09-27。继续复用已缓存VBZ站棚73、广告598/1020、长椅562/570/328、售票机577和信息设施2354。售票机0.9005093×0.2797088m平面来自官方几何；机器高度、交互面制造、广告原创图文和长椅细分是推断。长椅562与广告598共享源锚点，不能按独立居中物件重叠放置。信息设施type85未确认具体型式，当前小型共架Bellevue牌明确为推断，不显示虚构的实时交通信息。几何存在不代表设施已经可操作。

r12保留真实凹形AV355/456/457站岛，未用会多覆盖119.298629m²相邻道路的凸包。独立实图失败后，实际原生像素射线确认20处近残片来自偏出屋面但仍在已建地坪之内的摄影三角形；原屋面几何本身不因此改变。r13准备只对已建候车带139.244560m²内进行局部替换，北侧另一个高杆保留；这是重建边界推断，不是新增现场测绘。oak_veneer_01为原有CC0材料代理，实际查看纹理后确认长纹沿V，拟修正木条UV，不新增材料授权或实测木种声明。当前r13仍待原生实施和实图，详见planning/BELLEVUE_BANK_TRAM_SHELTER_CN.md。
